...
% 本文件的内容2025年2月由 嘉应学院 hkk 制作

\documentclass{jyuthesis} 

%\renewcommand{\normalsize}{\zihao{-4}\kai} % 正文字体使用楷体小四
\usepackage{gbt7714}                       % 采用国标参考文献样式
\bibliographystyle{gbt7714-numerical}      % 参考文献按照引用先后顺序编码
...
\newcommand{\makecover}{
    ...
    \begin{center}
        ...
        \begin{figure}[ht]
          \centering
          \includegraphics[width=0.7\linewidth]{fig_jyu_logo.pdf}
        \end{figure}
        ...
        \vspace{1.5cm}
        {\zihao{2} \bfseries （2025届）} \\
        \vspace{2cm}
                      
        \begin{spacing}{2.0}
        {\zihao{-3} 
            \textbf{题\quad\quad 目：} \uline{\makebox[9cm]{\mytitle}} \\
            %\textbf{\quad\quad\quad\quad\quad} \uline{\makebox[9cm]{标题太长的话，可以拆成两行}} \\
            \textbf{姓\quad\quad 名：} \uline{\makebox[9cm]{张三}} \\
            \textbf{学\quad\quad 号：} \uline{\makebox[9cm]{123456789}} \\
            \textbf{学\quad\quad 院：} \uline{\makebox[9cm]{数学学院}} \\
            \textbf{专\quad\quad 业：} \uline{\makebox[9cm]{数学与应用数学}} \\
            \textbf{指导老师：} \uline{\makebox[9cm]{李四}} \\
            \textbf{申请学位：} \uline{\makebox[9cm]{理学学士}} \\
        }
        \end{spacing}
        
        \vspace{3cm}
        {\zihao{3} \bfseries  嘉\quad 应\quad 学\quad 院\quad 教\quad 务\quad 处\quad 制} \\
    \end{center}
    \newpage
}

\title{本科毕业论文\LaTeX{}模板的使用方法}  % 这里设置的标题自动作为封面的题目 \mytitle 和 正文的页眉。

\makeatletter
\let\mytitle\@title % 将 \title{} 里面的内容赋值给自定义的 \mytitle 变量
\let\oldsection\section
\renewcommand\section{\clearpage\oldsection}  % 重新定义 \section 命令，每章强制新第一页开始
\makeatother
...
\pagenumbering{Roman}  % 摘要页的页码用罗马数字 I, II
\begin{center}
    {\zihao{3} \bfseries 摘要}
\end{center}
...
\noindent{\bf 关键词：} {毕业论文，论文排版，参考文献}

% Abstract 页
\newpage
%\thispagestyle{empty} % Abstract 页不需要页码
\begin{center}
...
\noindent{\bf Keywords:}$\quad$ {Undergraduate Thesis, Paper Typesetting, References}

% 目录页
\newpage
%\thispagestyle{empty} % 目录页不需要页码
\tableofcontents

\newpage  % 强制换新的一页
\setcounter{page}{1} % 页码开始为1
\pagenumbering{arabic}  % 正文的页码用阿拉伯数字 1, 2, 3
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{fig_track.png}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=1.0\textwidth]{fig_cnki.png}
